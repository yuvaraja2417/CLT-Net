# -*- coding: utf-8 -*-
"""QPCA-CLT STL  Scaling.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16JZteyPbtnN5QsxPtfVuSRUMEavHlh53
"""
...
crop = pd.read_csv("/content/LSTM-Multivariate_pollution.csv")
...
start_date = pd.to_datetime("1/2/2010 0:00")
end_date = start_date + pd.DateOffset(hours=len(data) - 1)
date_range = pd.date_range(start=start_date, end=end_date, freq='H')

# Set the index of the data DataFrame
data.index = date_range
# Fill Missing Values
data.fillna(method='ffill', inplace=True)

# Compute Exponential Moving Average (EMA)
data['EMA_12'] = data['pollution'].ewm(span=12, adjust=False).mean()
...
plt.plot(y_test_rescaled[:100], label='Actual Pollution', marker='o')
plt.plot(y_pred_rescaled[:100], label='Predicted Pollution', marker='x')
plt.xlabel("Time")
plt.ylabel("Pollution Pollution Level")
plt.legend()
...
pip install numpy pandas tensorflow scikit-learn statsmodels matplotlib
...
# Apply PCA and get necessary values
scaler = StandardScaler()
x_scaled = scaler.fit_transform(x)
pca = PCA(n_components=min(3, x.shape[1]))  # Ensure at most 3 components for 3D plotting
x_pca = pca.fit_transform(x_scaled)
explained_variance = pca.explained_variance_ratio_
reconstructed_df = pd.DataFrame(scaler.inverse_transform(pca.inverse_transform(x_pca)), columns=x.columns)
reconstruction_errors = np.linalg.norm(x - reconstructed_df, axis=1)

# Plot all figures in one step
fig, axs = plt.subplots(2, 2, figsize=(14, 12))
fig.suptitle('PCA Analysis & Visualization', fontsize=16, fontweight='bold')

# Explained Variance Plot
axs[0, 0].plot(range(1, len(explained_variance) + 1), np.cumsum(explained_variance), marker='o', linestyle='--', color='b')
axs[0, 0].set_xlabel('Number of Components', fontsize=12, fontweight='bold')
axs[0, 0].set_ylabel('Cumulative Explained Variance', fontsize=12, fontweight='bold')
axs[0, 0].set_title('Explained Variance Plot', fontsize=14, fontweight='bold')
axs[0, 0].grid()

# Scree Plot
axs[0, 1].bar(range(1, len(explained_variance) + 1), explained_variance, alpha=0.7, color='c')
axs[0, 1].set_xlabel('Principal Component', fontsize=12, fontweight='bold')
axs[0, 1].set_ylabel('Variance Explained', fontsize=12, fontweight='bold')
axs[0, 1].set_title('Scree Plot', fontsize=14, fontweight='bold')
axs[0, 1].grid()

# PCA Biplot (PC1 vs. PC2)
axs[1, 0].scatter(x_pca[:, 0], x_pca[:, 1], alpha=0.7, edgecolors='k')
axs[1, 0].set_xlabel('Principal Component 1', fontsize=12, fontweight='bold')
axs[1, 0].set_ylabel('Principal Component 2', fontsize=12, fontweight='bold')
axs[1, 0].set_title('PCA Biplot (PC1 vs. PC2)', fontsize=14, fontweight='bold')
axs[1, 0].grid()

# Add feature vectors to the biplot
for i, feature in enumerate(x.columns):
    axs[1, 0].arrow(0, 0, pca.components_[0, i], pca.components_[1, i], color='r', alpha=0.5)
    axs[1, 0].text(pca.components_[0, i] * 1.2, pca.components_[1, i] * 1.2, feature, color='g', ha='center', fontsize=10, fontweight='bold')

# Reconstruction Error Distribution
axs[1, 1].hist(reconstruction_errors, bins=30, alpha=0.7, color='m')
axs[1, 1].set_xlabel('Reconstruction Error', fontsize=12, fontweight='bold')
axs[1, 1].set_ylabel('Frequency', fontsize=12, fontweight='bold')
axs[1, 1].set_title('Reconstruction Error Distribution', fontsize=14, fontweight='bold')
axs[1, 1].grid()

# Adjust layout
plt.tight_layout(rect=[0, 0, 1, 0.96])

# 3D PCA Scatter Plot (if 3 components are present)
if x_pca.shape[1] == 3:
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(x_pca[:, 0], x_pca[:, 1], x_pca[:, 2], alpha=0.7, edgecolors='k')
...
# Load ERP dataset
data = pd.read_csv("/content/ERP.csv")
features = ['Fp1']  # Selecting only Fp1 feature
data = data[features]
...
# prompt: for the above model give the  test prediction graph visualization code with all bolt fonts like lables, scales, axis, indexings all

import matplotlib.pyplot as plt

# Assuming y_test_rescaled and y_pred_rescaled are your actual and predicted values
plt.figure(figsize=(12, 6))
plt.plot(y_test_rescaled[:100], label='Actual Values', marker='o', linewidth=2, color='blue')
plt.plot(y_pred_rescaled[:100], label='Predicted Values', marker='x', linewidth=2, color='red')
plt.xlabel('Time', fontsize=12, fontweight='bold')
plt.ylabel('Fp1 Signal', fontsize=12, fontweight='bold')
plt.title('Actual vs. Predicted Fp1 Signal', fontsize=14, fontweight='bold')
...
plt.figure(figsize=(12, 6))
plt.plot(data.index[-len(y_test_rescaled):], y_test_rescaled, label='Actual Pollution', color='blue', linewidth=2, )
plt.plot(data.index[-len(y_pred_rescaled):], y_pred_rescaled, label='Predicted Pollution', color='red', linestyle='dashed', linewidth=2, )
plt.xlabel('Time', fontsize=12, fontweight='bold')
plt.ylabel('Pollution Level', fontsize=12, fontweight='bold')
plt.title('Hybrid CLT-Net Test Predictions vs Actual', fontsize=14, fontweight='bold')
plt.legend(fontsize=12, loc='best')
plt.xticks(fontweight='bold')
plt.yticks(fontweight='bold')
plt.grid(alpha=0.5)
...
# Load dataset (Assuming 'crop' DataFrame is preloaded)
data = crop.copy()
...
pca_results = {}
for n in [data.shape[1], 4, 2, 1]:
    pca_results[n] = apply_pca(data_scaled, n)

# Explained Variance Plot
plt.figure(figsize=(8, 5))
components = [data.shape[1], 4, 2, 1]
variance_values = [pca_results[n][2] for n in components]
plt.bar([str(n) + " components" for n in components], variance_values, color=['b', 'g', 'r', 'c'])
plt.xlabel("Number of PCA Components")
plt.ylabel("Explained Variance")
plt.title("Explained Variance for Different PCA Components")
plt.show()

# 2D Scatter Plot (for PCA with 2 components)
if 2 in pca_results:
    pca_2d, _, _ = pca_results[2]
...
# 3D Scatter Plot (for PCA with 4 components, using first 3 for visualization)
if 4 in pca_results:
    from mpl_toolkits.mplot3d import Axes3D
    pca_3d, _, _ = pca_results[4]
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection='3d')
...
for n in components:
    _, pca_model, _ = pca_results[n]
    reconstructed = pca_model.inverse_transform(pca_results[n][0])
    reconstructed = scaler.inverse_transform(reconstructed)
    error = np.linalg.norm(data.values - reconstructed, axis=1)
    reconstruction_errors.append(np.mean(error))

plt.figure(figsize=(8, 5))
plt.plot([str(n) for n in components], reconstruction_errors, marker='o', linestyle='-', color='red')
plt.xlabel("Number of PCA Components")
